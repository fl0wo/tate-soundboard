...
export function toCompressedTitle(title:string) {
  return title
    .toLowerCase()
    .trim()
    .split(' ')
...
export class CacheService {
  ...
  constructor(
    ...
    this._ready = new Promise((resolve, reject) => {
      this.platform.ready()
        .then(() => {
          this.fileTransfer = this.ft.create();
  ...
  hasInCache(sound: any): boolean {
    return this.getCache().findIndex(cachedSound => cachedSound.title === toCompressedTitle(sound.title)) > -1;
  }
  ...
  addToCache(sound: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      console.log('DOWNLOADING ON',this.file.externalDataDirectory , " + ", toCompressedTitle(sound.title))
      this.fileTransfer.download(sound.src, this.file.externalDataDirectory + toCompressedTitle(sound.title))
        .then(entry => {
          ...
          console.log('incoming sound from s3', JSON.stringify(cachedSound))
  ...
  removeFromCache(sound: any): Promise<any> {
    return new Promise((resolve, reject) => {
      const index = this.getCache().findIndex(cachedSound => cachedSound.title === toCompressedTitle(sound.title));
      if (index === -1) {
      ...
      this.storage.remove('cache:' + toCompressedTitle(sound.title))
        .catch(error => reject(error));

      /* Remove the sound from filesystem */

      return this.file.removeFile(this.file.externalDataDirectory, toCompressedTitle(sound.title));
    });
  }
  ...
  getFromCache(sound: any): any {
    ...
    return this.getCache()[this.getCache().findIndex(cachedSound => cachedSound.title === toCompressedTitle(sound.title))];
  }
}
